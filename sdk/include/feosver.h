...
static inline bool FeOS_VersionAtLeast(int minorVer, int majorVer)
{
	return FeOS_GetVersion() >= FEOS_VPACK_MAKE(minorVer, majorVer);
}

static inline bool FeOS_VersionUpTo(int minorVer, int majorVer)
{
	return FeOS_GetVersion() <= FEOS_VPACK_MAKE(minorVer, majorVer);
}

static inline bool FeOS_VersionLaterThan(int minorVer, int majorVer)
{
	return FeOS_GetVersion() > FEOS_VPACK_MAKE(minorVer, majorVer);
}

static inline bool FeOS_VersionEarlierThan(int minorVer, int majorVer)
{
	return FeOS_GetVersion() < FEOS_VPACK_MAKE(minorVer, majorVer);
}
