/*---------------------------------------------------------------------------------
	...
	feos3d.h -- Video API vaguely similar to OpenGL (FeOS version)
...
typedef hword_t fixed12d3; /*!< \brief Used for depth (glClearDepth, glCutoffDepth) */
...
typedef struct s_vramBlock {
	...
	byte_t *lastExaminedAddr;
	word_t lastExaminedSize;

	glDynArray blockPtrs;
	glDynArray deallocBlocks;
...
typedef struct gl_hidden_globals {
	...
	// texture globals
	glDynArray texturePtrs;		// Pointers to each individual texture
	glDynArray palettePtrs;		// Pointers to each individual palette

	glDynArray deallocTex;		// Preserves deleted texture names for later use with glGenTextures
	glDynArray deallocPal;		// Preserves deleted palette names
