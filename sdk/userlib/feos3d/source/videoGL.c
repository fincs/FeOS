...
FEOS_EXPORT gl_hidden_globals glGlobalData;



// This returns the pointer to the globals for videoGL
FEOS_EXPORT gl_hidden_globals* glGetGlobals() {
	return &glGlobalData;
}

//---------------------------------------------------------------------------------
FEOS_EXPORT void glRotatef32i(int angle, int32 x, int32 y, int32 z) {
//---------------------------------------------------------------------------------
	int32 axis[3];
	...
	barrierAccess(MATRIX_MULT3x3 = cosine + mulf32(one_minus_cosine, mulf32(axis[0], axis[0])));
	barrierAccess(MATRIX_MULT3x3 = mulf32(one_minus_cosine, mulf32(axis[0], axis[1])) + mulf32(axis[2], sine));
	barrierAccess(MATRIX_MULT3x3 = mulf32(mulf32(one_minus_cosine, axis[0]), axis[2]) - mulf32(axis[1], sine));

	barrierAccess(MATRIX_MULT3x3 = mulf32(mulf32(one_minus_cosine, axis[0]),  axis[1]) - mulf32(axis[2], sine));
	barrierAccess(MATRIX_MULT3x3 = cosine + mulf32(mulf32(one_minus_cosine, axis[1]), axis[1]));
	barrierAccess(MATRIX_MULT3x3 = mulf32(mulf32(one_minus_cosine, axis[1]), axis[2]) + mulf32(axis[0], sine));

	barrierAccess(MATRIX_MULT3x3 = mulf32(mulf32(one_minus_cosine, axis[0]), axis[2]) + mulf32(axis[1], sine));
	barrierAccess(MATRIX_MULT3x3 = mulf32(mulf32(one_minus_cosine, axis[1]), axis[2]) - mulf32(axis[0], sine));
	barrierAccess(MATRIX_MULT3x3 = cosine + mulf32(mulf32(one_minus_cosine, axis[2]), axis[2]));
}
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glMaterialf(GL_MATERIALS_ENUM mode, rgb color) {
//---------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glTexCoord2f32(int32 u, int32 v) { 
//---------------------------------------------------------------------------------
	int x, y;
	gl_texture_data *tex = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs, glGlob->activeTexture );
	if( tex ) {
...
vramBlock_init( s_vramBlock *mb ) {
	...
	glDynArrayInit( &mb->blockPtrs, 16 );
	glDynArrayInit( &mb->deallocBlocks, 16 );
	int i;
	for( i = 0; i < 16; i++ ) {
		glDynArraySet( &mb->blockPtrs, i, (void*)0 );
		glDynArraySet( &mb->deallocBlocks, i, (void*)0 );
	}	
}
...
vramBlock_terminate( s_vramBlock *mb ) {
	...
	glDynArrayDelete( &mb->deallocBlocks );
	glDynArrayDelete( &mb->blockPtrs );
}
...
vramBlock_allocateSpecial( s_vramBlock *mb, uint8 *addr, uint32 size ) {
	...
	if( newBlock ) { // with current implementation, it should never be false if it gets to here
		...
		if( mb->deallocCount )
			curBlock = (uint32)glDynArrayGet( &mb->deallocBlocks, mb->deallocCount-- );
		else
			curBlock = mb->blockCount++;
		glDynArraySet( &mb->blockPtrs, curBlock, (void*)newBlock );
...
vramBlock_deallocateBlock( s_vramBlock *mb, uint32 index ) {
	...
	struct s_SingleBlock *block = (struct s_SingleBlock*)glDynArrayGet( &mb->blockPtrs, index );
	if( block && vramBlock__deallocateBlock( mb, block )) {
		glDynArraySet( &mb->blockPtrs, index, (void*)0x0 );
		glDynArraySet( &mb->deallocBlocks, ++mb->deallocCount, (void*)index );
		return 1;
	}
	return 0;
}
...
vramBlock_getAddr( s_vramBlock *mb, uint32 index ) {
	...
	if(( getBlock = (struct s_SingleBlock*)glDynArrayGet( &mb->blockPtrs, index )))
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glInit_C(void) {
//---------------------------------------------------------------------------------
	int i;
	...
	glDynArrayInit( &glGlob->texturePtrs, 16 );
	glDynArrayInit( &glGlob->palettePtrs, 16 );
	glDynArrayInit( &glGlob->deallocTex, 16 );
	glDynArrayInit( &glGlob->deallocPal, 16 );
	
	for(i = 0; i < 16; i++) {
		glDynArraySet( &glGlob->texturePtrs, i, (void*)0 );
		glDynArraySet( &glGlob->palettePtrs, i, (void*)0 );
		glDynArraySet( &glGlob->deallocTex, i, (void*)0 );
		glDynArraySet( &glGlob->deallocPal, i, (void*)0 );
	}
...
//---------------------------------------------------------------------------------
{
	...
	glDynArrayDelete(&glGlob->texturePtrs);
	glDynArrayDelete(&glGlob->palettePtrs);
	glDynArrayDelete(&glGlob->deallocTex);
	glDynArrayDelete(&glGlob->deallocPal);

	memset(glGlob, 0, sizeof(gl_hidden_globals));
}

//---------------------------------------------------------------------------------
FEOS_EXPORT void glResetTextures(void) {
//---------------------------------------------------------------------------------
	int i;
	...
	for(i = 0; i < (int)glGlob->texturePtrs.cur_size; i++) {
		gl_texture_data* texture = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs, i );
		if( texture ) {
			free( texture );
			glDynArraySet(&glGlob->texturePtrs, i, (void*)0 );
		}
	...
	for( i = 0; i < (int)glGlob->palettePtrs.cur_size; i++ ) {
		gl_palette_data* palette = (gl_palette_data*)glDynArrayGet( &glGlob->palettePtrs, i );
		if( palette ) {
			free( palette );
			glDynArraySet( &glGlob->palettePtrs, i, (void*)0 );
		}
...
void removePaletteFromTexture( gl_texture_data *tex ) {
	if( tex ) {
		gl_palette_data *palette = (gl_palette_data*)glDynArrayGet( &glGlob->palettePtrs, tex->palIndex );
		if( palette->connectCount ) {
			if( !(--palette->connectCount)) {
				glDynArraySet( &glGlob->deallocPal, ++glGlob->deallocPalSize, (void*)tex->palIndex );
				...
				glDynArraySet( &glGlob->palettePtrs, tex->palIndex, (void*)0 );	
			}
			tex->palIndex = 0;
		}
	}
...
FEOS_EXPORT int glGenTextures(int n, int *names) {
//---------------------------------------------------------------------------------
	int index = 0;
	...
	for(index = 0; index < n; index++) {
		...
		if( glGlob->deallocTexSize  ) // Use previously deleted texture names
			names[ index ] = (uint32)glDynArrayGet( &glGlob->deallocTex, glGlob->deallocTexSize-- );
		else
			names[ index ] = glGlob->texCount++;
		glDynArraySet( &glGlob->texturePtrs, names[ index ], (void*)texture  );
	}
...
FEOS_EXPORT int glDeleteTextures( int n, int *names ) {
//---------------------------------------------------------------------------------
	int index = 0;
	for(index = 0; index < n; index++) {
		if( names[ index ] != 0 ) {
			if( glGlob->deallocTexSize == MAX_TEXTURES || names[ index ] >= MAX_TEXTURES ) // This still needed?
				return 0;
			glDynArraySet( &glGlob->deallocTex, ++glGlob->deallocTexSize, (void*)names[ index ] );
			gl_texture_data *texture = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs, names[ index ] );
			if( texture ) {
			...
			glDynArraySet( &glGlob->texturePtrs, names[ index ], (void*)0 );
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glBindTexture(int target, int name) {
//---------------------------------------------------------------------------------
	...
	// name exist?
	if(( tex = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs, name ))) {
		GFX_TEX_FORMAT = tex->texFormat;
		glGlob->activeTexture = name;
		// Set palette if exists
		if( tex->palIndex ) {
			gl_palette_data *pal = (gl_palette_data*)glDynArrayGet( &glGlob->palettePtrs, tex->palIndex );
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glColorTableEXT( int target, int empty1, uint16 width, int empty2, int empty3, const uint16* table ) {
//---------------------------------------------------------------------------------
	if( glGlob->activeTexture ) {
		gl_texture_data *texture = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs, glGlob->activeTexture );
		...
		if( checkAddr ) {
			...
			if( glGlob->deallocPalSize )
				texture->palIndex = (uint32)glDynArrayGet( &glGlob->deallocPal, glGlob->deallocPalSize-- );
			else
				texture->palIndex = glGlob->palCount++;
			glDynArraySet( &glGlob->palettePtrs, texture->palIndex, (void*)palette );
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glColorSubTableEXT( int target, int start, int count, int empty1, int empty2, const uint16* data ) {
	if( count > 0 && glGlob->activePalette ) {
		gl_palette_data *palette = (gl_palette_data*)glDynArrayGet( &glGlob->palettePtrs, glGlob->activePalette );
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glGetColorTableEXT( int target, int empty1, int empty2, uint16* table ) {
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glAssignColorTable( int target, int name ) {
//---------------------------------------------------------------------------------
	// Allow assigning from a texture different from the active one
	if( glGlob->activeTexture && glGlob->activeTexture != name ) {
		gl_texture_data *texture = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs , glGlob->activeTexture );
		gl_texture_data *texCopy = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs , name );
		gl_palette_data *palette;
		if( texture->palIndex ) // Remove prior palette if exists
			removePaletteFromTexture( texture );
		...
		if( texCopy && texCopy->palIndex ) {
			...
			palette = (gl_palette_data*)glDynArrayGet( &glGlob->palettePtrs, texture->palIndex );
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glTexParameter( int target, int param ) {
//---------------------------------------------------------------------------------
	if( glGlob->activeTexture ) {
		gl_texture_data *tex = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs , glGlob->activeTexture );
...
//
//---------------------------------------------------------------------------------
FEOS_EXPORT void* glGetTexturePointer(	int name ) {
//---------------------------------------------------------------------------------
	gl_texture_data *tex = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs, name );
	if( tex )
		return tex->vramAddr;
	else
		return NULL;
...
//---------------------------------------------------------------------------------
FEOS_EXPORT u32 glGetTexParameter() {
//---------------------------------------------------------------------------------
	if( glGlob->activeTexture ) {
		gl_texture_data *tex = (gl_texture_data*)glDynArrayGet( &glGlob->texturePtrs, glGlob->activeTexture );
...
//---------------------------------------------------------------------------------
FEOS_EXPORT void glGetColorTableParameterEXT( int target, int pname, int * params ) {
//---------------------------------------------------------------------------------
	if( glGlob->activePalette ) {
		gl_palette_data *pal = (gl_palette_data*)glDynArrayGet( &glGlob->palettePtrs, glGlob->activePalette );
...
//---------------------------------------------------------------------------------
FEOS_EXPORT int glTexImage2D(int target, int empty1, GL_TEXTURE_TYPE_ENUM type, int sizeX, int sizeY, int empty2, int param, const void* texture) {
//---------------------------------------------------------------------------------
	uint32 size = 0;
